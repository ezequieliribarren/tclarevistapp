    ...
    try {
        ...
        for (const tramo in tramos) {
            ...
            $('.titulo_grande_blanco3').each((i, row) => {
                ...
                // Verifica si hay suficientes columnas para procesar
                if (columns.length >= 5) {
                    ...
                    // Comprueba si hay suficientes columnas para procesar el tiempo y la diferencia
                    if (columns.length >= 6) {
            ...
            datosPorTramo[tramo] = tablaPosiciones.slice(2);
        }
