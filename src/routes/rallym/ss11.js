...
async function p11() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[30] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[30].v);

    const urlsRally2 = datos[0].data
      .filter(fila => fila.c[31] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[31].v);
