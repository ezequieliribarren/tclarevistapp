...
async function p29() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[66] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[66].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[67] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[67].v);
