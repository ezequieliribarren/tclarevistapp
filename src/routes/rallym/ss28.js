...
async function p28() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[64] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[64].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[65] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[65].v);
