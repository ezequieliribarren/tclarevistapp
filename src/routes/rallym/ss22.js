...
async function p22() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[52] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[52].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[53] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[53].v);
