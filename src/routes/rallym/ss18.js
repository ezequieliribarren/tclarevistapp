...
async function p18() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[44] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[44].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[45] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[45].v);
