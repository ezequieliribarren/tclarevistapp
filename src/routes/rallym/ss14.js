...
async function p14() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[36] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[36].v);

    const urlsRally2 = datos[0].data
      .filter(fila => fila.c[37] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[37].v);
