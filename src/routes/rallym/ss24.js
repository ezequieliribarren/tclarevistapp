...
async function p24() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[56] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[56].v);

    const urlsRally2 = datos[0].data
      .filter(fila => fila.c[57] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[57].v);
