...
async function p19() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[46] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[46].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[47] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[47].v);
