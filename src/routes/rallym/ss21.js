...
async function p21() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[50] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[50].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[51] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[51].v);
