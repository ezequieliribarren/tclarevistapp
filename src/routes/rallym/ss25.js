...
async function p25() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[58] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[58].v);

    const urlsRally2 = datos[0].data
      .filter(fila => fila.c[59] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[59].v);
