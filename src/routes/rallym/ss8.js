...
async function p8() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[24] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[24].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[25] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[25].v);
