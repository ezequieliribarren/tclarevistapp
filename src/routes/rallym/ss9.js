...
async function p9() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[26] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[26].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[27] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[27].v);
