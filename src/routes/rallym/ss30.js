...
async function p30() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[68] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[68].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[69] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[69].v);
