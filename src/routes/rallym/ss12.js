...
async function p12() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[32] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[32].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[33] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[33].v);
