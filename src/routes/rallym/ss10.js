...
async function p10() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[28] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[28].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[29] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[29].v);
