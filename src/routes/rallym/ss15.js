...
async function p15() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[38] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[38].v);

    const urlsRally2 = datos[0].data
      .filter(fila => fila.c[39] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[39].v);
