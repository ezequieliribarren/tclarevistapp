...
async function p20() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[48] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[48].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[49] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[49].v);
