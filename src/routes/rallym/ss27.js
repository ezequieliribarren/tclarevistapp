...
async function p27() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[62] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[62].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[63] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[63].v);
