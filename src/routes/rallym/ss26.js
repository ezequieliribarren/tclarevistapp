...
async function p26() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[60] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[60].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[61] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[61].v);
