...
async function p23() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[54] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[54].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[55] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[55].v);
