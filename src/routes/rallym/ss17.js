...
async function p17() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[42] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[42].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[43] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[43].v);
