...
async function p6() {
  try {
    ...
    const urlsRally = datos[0].data
      .filter(fila => fila.c[20] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[20].v);

      const urlsRally2 = datos[0].data
      .filter(fila => fila.c[21] !== null) // Filtrar las filas con valor null
      .map(fila => fila.c[21].v);
